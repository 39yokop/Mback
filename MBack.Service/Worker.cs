...
private void ReloadAndRestartWatchers()
    {
        ...
        // ★修正ポイント: 設定がなくても終了(return)せず、空のリストとして扱う
        ...
        else
        {
            ...
            // ここで return しても、ExecuteAsync のループは生きているので
            // ConfigWatcher と TriggerWatcher は働き続けます！
            return;
        }

        // 監視のセットアップ（設定がある場合のみここに来る）
    ...
    private async Task PerformInitialSyncAsync()
    {
        ...
        foreach (var pair in currentPairs)
        {
            try
            {
                ...
                // 2. ファイルの同期（コピー）
                var sourceFiles = Directory.GetFiles(pair.Source, "*", SearchOption.AllDirectories);
