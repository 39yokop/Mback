...
public partial class Form1 : Form
{
    ...
    private void OnViewLogClick(object? s, EventArgs e)
    {
        ...
        {
            string dir = Path.GetDirectoryName(_jsonPath) ?? "";
            string logPath = Path.Combine(dir, "Logs", $"log-{DateTime.Now:yyyyMMdd}.txt");
            if (!File.Exists(logPath)) { MessageBox.Show("今日のログはまだありません。\n" + logPath); return; }

            Process.Start(new ProcessStartInfo { FileName = "powershell.exe", Arguments = $"-NoExit -Command \"Get-Content '{logPath}' -Wait -Encoding UTF8\"", UseShellExecute = true });
        }
        catch (Exception ex) { MessageBox.Show("エラー: " + ex.Message); }
    }
}