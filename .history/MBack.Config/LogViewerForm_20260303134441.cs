...
public class LogViewerForm : Form
{
    ...
    {
        this.Text = "MBack 履歴復元センター (SQLite・完全均等レイアウト版)";
        this.Size = new Size(1200, 750); // 少し幅を広げてゆとりを持たせた
        ...
        // ★重要：フォームが表示される直前のイベントでレイアウトを確定させる
        this.Load += (s, e) => FixSplitterLayout();
    ...
    private void SetupLayout()
    {
        ...
        _splitSub.Panel1MinSize = 200;
        _splitSub.Panel2MinSize = 200;
    ...
    /// <summary>
    /// ★仕切りの位置を「強制的に」3等分に計算し直すプロの技
    /// </summary>
    private void FixSplitterLayout()
    {
        // 1. まず全体を3等分（左:1/3、右:2/3）
        _splitMain.SplitterDistance = this.ClientSize.Width / 3;
        
        // 2. 残った右側のスペースをさらに半分（中央:1/3、右:1/3）
        _splitSub.SplitterDistance = _splitMain.Panel2.Width / 2;
    }

    // --- DB連携・復元ロジック (既存の緻密なロジックを継承) ---
    ...
    private void RestoreFile(string? specificVersionPath, string? targetSourcePath = null)
    {
        ...
        if (MessageBox.Show($"復元しますか？\n{sourcePath}", "復元", MessageBoxButtons.YesNo) == DialogResult.Yes) {
            try {
