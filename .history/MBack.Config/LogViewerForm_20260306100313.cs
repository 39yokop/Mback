...
public class LogViewerForm : Form
{
    ...
    private void OnShowVersionsClick(object? sender, EventArgs e)
    {
        ...
        // ★修正：CS8602 警告解消（文字列による列検索は null の可能性があるため、インデックスで直接指定）
    ...
    private void OnOpenBackupFolderClick(object? sender, EventArgs e)
    {
        ...
        string? b = GetBackupPath(p); if (b != null) { string? f = Path.GetDirectoryName(b); if (!string.IsNullOrEmpty(f) && Directory.Exists(f)) Process.Start("explorer.exe", f); }
    }
