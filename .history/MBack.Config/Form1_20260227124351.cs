...
namespace MBack.Config
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            {
                try { File.Copy(oldPath, _jsonPath); } catch { }
            }
        ...
        private void SetupLayout()
        {
            ...
            InitButton(_btnService, "サービス: 確認中", OnServiceClick, 250);
        ...
        private void UpdateServiceState()
        {
            ...
            // ★ Worker.csが異常時に作成する emergency.txt を監視
            string emergencyFile = Path.Combine(_configDir, "emergency.txt");
            _lblEmergency.Visible = File.Exists(emergencyFile);
        }
        ...
        private void LoadWindowState()
        {
            try {
                ...
                if (key != null) {
                    int w = (int)(key.GetValue("Width") ?? 800); int h = (int)(key.GetValue("Height") ?? 550);
        ...
        private void OnAddClick(object? sender, EventArgs e)
        {
            ...
            if (form.ShowDialog() == DialogResult.OK) {
                _backupList.Add(form.Pair); UpdateGrid();
            }
        }
        ...
        private void OnEditClick(object? sender, EventArgs e)
        {
            ...
            if (form.ShowDialog() == DialogResult.OK) {
                _backupList[index] = form.Pair; UpdateGrid();
            }
        }
        ...
        private void OnDeleteClick(object? sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("設定を削除してもよろしいですか？", "削除確認", MessageBoxButtons.YesNo) == DialogResult.Yes) {
                _backupList.RemoveAt(_grid.SelectedRows[0].Index); UpdateGrid();
            }
        }
        ...
        private void OnExclusionClick(object? sender, EventArgs e)
        {
            ...
            if (form.ShowDialog() == DialogResult.OK) { _globalExclusions = form.Exclusions; SaveSettings(); }
        }

        private void OnMailClick(object? sender, EventArgs e)
        {
            using var form = new MailSettingsForm(_mailConfig);
            if (form.ShowDialog() == DialogResult.OK) { _mailConfig = form.Config; SaveSettings(); }
        }
        ...
        private void OnServiceClick(object? sender, EventArgs e)
        {
            try {
                ...
                if (sc.Status == System.ServiceProcess.ServiceControllerStatus.Running) {
                    if (MessageBox.Show("サービスを停止しますか？\n（緊急停止を解除する場合も一度停止してください）", "確認", MessageBoxButtons.YesNo) == DialogResult.Yes) {
        ...
        private void UpdateGrid()
        {
            ...
            foreach (var p in _backupList) {
                string maskPass = string.IsNullOrEmpty(p.Password) ? "" : "***";
                string userDisp = string.IsNullOrEmpty(p.UserName) ? "" : $"{p.UserName} ({maskPass})";
                _grid.Rows.Add(p.Source, p.Destination, userDisp, Path.GetFileName(p.PreCommand));
            }
        }
    }

    // =========================================================================
    // ★ 新規追加: バックアップ先とNAS認証を細かく設定するサブフォーム
    // =========================================================================
