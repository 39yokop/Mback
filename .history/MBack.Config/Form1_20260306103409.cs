...
namespace MBack.Config
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            this.Text = "MBack 設定ツール (v2.0 - Layout Fixed Edition)";
        ...
        {
            // ★絶対防衛策：画面全体を親の TableLayoutPanel で 3段（警告・タブ・フッター）に物理分割する
            var rootLayout = new TableLayoutPanel {
                Dock = DockStyle.Fill, ColumnCount = 1, RowCount = 3,
                Margin = new Padding(0), Padding = new Padding(0)
            };
            rootLayout.RowStyles.Add(new RowStyle(SizeType.AutoSize));    // 1段目: 警告ラベル（非表示時は高さ0になる）
            rootLayout.RowStyles.Add(new RowStyle(SizeType.Percent, 100F)); // 2段目: タブコントロール（残りの広さを全確保）
            rootLayout.RowStyles.Add(new RowStyle(SizeType.Absolute, 65F)); // 3段目: フッター（絶対に高さ65pxを死守）

            // --- 1. 緊急停止警告ラベル ---
            _lblEmergency.Text = "【警告】異常検知により緊急停止中！下部の復旧ボタンを押してください";
            _lblEmergency.BackColor = Color.Red; _lblEmergency.ForeColor = Color.White;
            _lblEmergency.Dock = DockStyle.Fill; _lblEmergency.Height = 35;
            ...
            _lblEmergency.Margin = new Padding(0);

            // --- 2. サービス常駐フッター ---
            var footerPanel = new TableLayoutPanel { 
                Dock = DockStyle.Fill, BackColor = Color.FromArgb(235, 240, 245),
                ColumnCount = 3, RowCount = 1, Margin = new Padding(0), Padding = new Padding(5)
            };
            footerPanel.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute, 180)); // 左: ステータス表示
            footerPanel.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 100));  // 中央: ボタンエリア
            footerPanel.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute, 150)); // 右: 保存して閉じる
            ...
            var centerFlow = new FlowLayoutPanel { 
                AutoSize = true, WrapContents = false, Anchor = AnchorStyles.None, 
            ...
            footerPanel.Controls.Add(_lblStatus, 0, 0);
            footerPanel.Controls.Add(centerFlow, 1, 0);
            footerPanel.Controls.Add(_btnSave, 2, 0);
            ...
            // タブ1の中もTableLayoutPanelで上下に物理分割（重なりを完全排除）
            var tab1Layout = new TableLayoutPanel { Dock = DockStyle.Fill, ColumnCount = 1, RowCount = 2, Margin = new Padding(0) };
            tab1Layout.RowStyles.Add(new RowStyle(SizeType.Percent, 100F)); // 上部: グリッド
            tab1Layout.RowStyles.Add(new RowStyle(SizeType.Absolute, 50F)); // 下部: ボタン群
            ...
            var tab1BtnPanel = new TableLayoutPanel { 
                Dock = DockStyle.Fill, ColumnCount = 2, RowCount = 1, Margin = new Padding(0), Padding = new Padding(2) 
            };
            tab1BtnPanel.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 100F)); // 左のボタン群
            tab1BtnPanel.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute, 200F)); // 右の除外ボタン

            var leftFlow = new FlowLayoutPanel { Dock = DockStyle.Fill, WrapContents = false, FlowDirection = FlowDirection.LeftToRight, Margin = new Padding(0) };
            ...
            InitButton(_btnExclusion, "🚫 グローバル除外設定", OnExclusionClick, 190);
            _btnExclusion.Anchor = AnchorStyles.Right;
            
            tab1BtnPanel.Controls.Add(leftFlow, 0, 0);
            tab1BtnPanel.Controls.Add(_btnExclusion, 1, 0);
            
            tab1Layout.Controls.Add(_grid, 0, 0);
            tab1Layout.Controls.Add(tab1BtnPanel, 0, 1);
            ...
            Action<Button, string, Color, EventHandler> setupTile = (btn, text, color, handler) => {
                btn.Text = text; btn.BackColor = color; btn.Dock = DockStyle.Fill; btn.Margin = new Padding(10);
            ...
            // ★全ての要素をルートレイアウトに登録
            rootLayout.Controls.Add(_lblEmergency, 0, 0);
        ...
        {
            btn.Text = text; btn.Width = width; btn.Height = 35; btn.Click -= handler; btn.Click += handler;
        }
