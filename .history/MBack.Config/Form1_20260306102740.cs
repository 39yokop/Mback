...
namespace MBack.Config
{
    public partial class Form1 : Form
    {
        ...
        private void SetupLayout()
        {
            ...
            // --- 2. サービス常駐フッター（★TableLayoutPanelで絶対に重ならないように修正） ---
            var footerPanel = new TableLayoutPanel { 
                Dock = DockStyle.Bottom, Height = 65, BackColor = Color.FromArgb(235, 240, 245),
                ColumnCount = 3, RowCount = 1, Padding = new Padding(5)
            };
            footerPanel.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute, 180)); // 左：ステータス用固定幅
            footerPanel.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 100));  // 中央：サービスボタン用（残り全部）
            footerPanel.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute, 150)); // 右：保存ボタン用固定幅
            ...
            // ★タブ1下部もTableLayoutPanel化して、ボタンが消える現象を物理的に阻止
            var tab1BtnPanel = new TableLayoutPanel { 
                Dock = DockStyle.Bottom, Height = 50, ColumnCount = 2, RowCount = 1, Padding = new Padding(2) 
            };
            tab1BtnPanel.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 100)); // 左：追加等のボタン群
            tab1BtnPanel.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute, 200)); // 右：除外設定ボタン

            var leftFlow = new FlowLayoutPanel { Dock = DockStyle.Fill, WrapContents = false, FlowDirection = FlowDirection.LeftToRight };
            ...
            _btnExclusion.Dock = DockStyle.Right;
            ...
            // ★ダッシュボードの余白を調整し、潰れないように修正
            var dashGrid = new TableLayoutPanel { Dock = DockStyle.Fill, Padding = new Padding(20), RowCount = 2, ColumnCount = 2 };
