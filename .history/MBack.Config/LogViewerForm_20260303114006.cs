...
public class LogViewerForm : Form
{
    ...
    // ★今回の目玉：ドラッグで幅を変えられるコンテナ
    private SplitContainer _splitMain = new(); // 左側(コピー)と右側(その他)を分ける
    private SplitContainer _splitSub = new();  // 右側の中で(ゴミ箱)と(エラー)を分ける
    ...
    {
        this.Text = "MBack 履歴復元センター (SQLite・可変レイアウト版)";
    ...
    private void SetupLayout()
    {
        ...
        // --- メインレイアウト（SplitContainerを入れ子にして3分割を実現） ---
        
        // 1. まず「コピー済み」と「それ以外（ゴミ箱・エラー）」を左右に分ける
        _splitMain.Dock = DockStyle.Fill;
        _splitMain.SplitterDistance = 440; // 初期の仕切り位置
        _splitMain.BorderStyle = BorderStyle.Fixed3D;

        // 2. 右側のパネルの中に、さらに「ゴミ箱」と「エラー」を左右に分けるスプリッターを入れる
        _splitSub.Dock = DockStyle.Fill;
        _splitSub.SplitterDistance = 320; // 初期の仕切り位置
        ...
        // サブスプリッターの左側に「ゴミ箱」、右側に「エラーログ」を入れる
