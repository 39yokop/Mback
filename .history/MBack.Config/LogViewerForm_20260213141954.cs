...
public class LogViewerForm : Form
{
    ...
    private List<BackupPair> _backupPairs = new(); // パス計算用
    ...
    public LogViewerForm()
    {
        ...
        // ※ 本番運用では ProgramData に変更することを推奨しますが、現在はそのままでOK
    ...
    private void OnRestoreClick(object? sender, EventArgs e)
    {
        ...
        // ノードからフルパスを復元（タグにパスを入れておくのが一番確実ですが、今回は階層から復元します）
        // ※ AddPathToTreeで Tag にフルパスを入れるよう改良します
    ...
    private string? GetBackupPath(string sourcePath)
    {
        foreach (var pair in _backupPairs)
        {
            ...
            if (sourcePath.StartsWith(pair.Source, StringComparison.OrdinalIgnoreCase))
            {
                ...
                // 例: Source="C:\Data", Dest="D:\Back", File="C:\Data\Img\1.jpg"
                // -> "D:\Back" + "\Img\1.jpg"
    ...
    // --- データ読み込みロジック (既存と同じですが Tag 設定を追加) ---
