...
public class LogViewerForm : Form
{
    ...
    private Button _btnRefresh = new(); // ★追加：更新ボタン
    ...
    {
        this.Text = "MBack 履歴復元センター (SQLite・可変・更新対応版)";
    ...
    private void SetupLayout()
    {
        ...
        _btnRefresh.Click += (s, e) => {
            LoadDateListFromDb(); // 日付リスト自体も更新（日付が変わった場合などのため）
            if (_dateSelector.SelectedItem != null) LoadLogFromDb(_dateSelector.SelectedItem.ToString());
        };
        ...
        // フォーム幅 1100 に対して、左パネルを約 360 に設定
        _splitMain.SplitterDistance = 360; 

        _splitSub.Dock = DockStyle.Fill;
        _splitSub.BorderStyle = BorderStyle.Fixed3D;
        // 残り 740 に対して、中央パネルを 370 に設定（これで右も 370 になる）
        _splitSub.SplitterDistance = 370; 
    ...
    private void LoadDateListFromDb()
    {
        ...
        try {
            ...
            while (reader.Read()) {
                _dateSelector.Items.Add(reader.GetString(0));
            }

            // 前に選んでいた日付があれば再選択、なければ最新
    ...
    private void RestoreFile(string? specificVersionPath, string? targetSourcePath = null)
    {
        ...
        if (backupPath == null || !File.Exists(backupPath)) { 
            MessageBox.Show("バックアップが見つかりません。"); 
            return; 
        }

        if (MessageBox.Show($"復元（上書き）しますか？\n{sourcePath}", "復元確認", MessageBoxButtons.YesNo) == DialogResult.Yes) {
            try {
