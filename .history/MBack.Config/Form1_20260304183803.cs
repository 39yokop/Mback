...
namespace MBack.Config
{
    public partial class Form1 : Form
    {
        ...
        private Button _btnRestart = new(); // ★追加：サービス再起動ボタン
        ...
        public Form1()
        {
            ...
            this.Size = new Size(800, 620); // ボタンが増えた分少し高くしました
        ...
        {
            _lblEmergency.Text = "【警告】ランサムウェア等の異常検知により緊急停止中！下部の復旧ボタンを押してください";
            ...
            // ★追加：サービス管理ボタン群を横並びにするためのパネル
            var servicePanel = new FlowLayoutPanel { Width = 350, Height = 45, FlowDirection = FlowDirection.LeftToRight };
            InitButton(_btnService, "サービス: 状態取得中", OnServiceClick, 200);
            InitButton(_btnRestart, "🔄 再起動", OnRestartClick, 90);
            _btnRestart.BackColor = Color.LightSkyBlue;
            servicePanel.Controls.AddRange(new Control[] { _btnService, _btnRestart });

            InitButton(_btnHelp, "MBack 使い方ガイド (Help)", OnHelpClick, 250);
            _btnHelp.BackColor = Color.LightCyan; 

            tab3Btn.Controls.AddRange(new Control[] { _btnAdvanced, _btnLog, servicePanel, _btnHelp });
        ...
        private void UpdateServiceState()
        {
            ...
            if (File.Exists(emergencyFile)) {
                ...
                // ★教官の神UI: ワンクリック復旧ボタンに変化
                _btnService.Text = "⚠️ 緊急停止を解除して再開";
                _btnService.BackColor = Color.Orange;
                _btnService.Width = 200;
                _btnRestart.Enabled = false; // 緊急時は再起動ボタンを無効化
            } else {
                _lblEmergency.Visible = false;
        ...
        private void LoadWindowState()
        {
            try {
                ...
                if (key != null) {
                    int w = (int)(key.GetValue("Width") ?? 800); int h = (int)(key.GetValue("Height") ?? 620);
        ...
        private void OnRestartClick(object? sender, EventArgs e)
        {
            try {
                ...
                _btnRestart.Text = "🔄 再起動"; _btnRestart.Enabled = true;
                UpdateServiceState();
            }
        }
