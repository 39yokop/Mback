...
public class LogViewerForm : Form
{
    ...
    {
        this.Text = "MBack 履歴復元センター (SQLite・完全均等・詳細表示版)";
    ...
    private void OnShowVersionsClick(object? sender, EventArgs e)
    {
        ...
        for (int i = 1; i <= MAX_FILE_HISTORY_LIMIT_DUMMY; i++) { // MAX_FILE_HISTORY(50)
             string v = $"{bPath}.v{i}"; if (File.Exists(v)) vers.Add(v); 
        }
    ...
    private const int MAX_FILE_HISTORY_LIMIT_DUMMY = 50;
