...
public class Worker : BackgroundService
{
    ...
    private const int RANSOMWARE_THRESHOLD = 100; // 100件のファイルが
    private const int RANSOMWARE_SECONDS = 60;    // 60秒間に変更されたら遮断

    public Worker(ILogger<Worker> logger)
    {
        _logger = logger;
        // ★修正：設定ファイルを共通のProgramDataから読み込む
    ...
    private bool CheckForRansomware()
    {
        ...
        // 閾値を超えたら発動！
        if (_eventTimes.Count >= RANSOMWARE_THRESHOLD)
        {
            _isCircuitBreakerTripped = true;
            _pendingBackups.Clear(); // 待機キューを全削除して被害を防ぐ
            
            string msg = $"【緊急警告】{RANSOMWARE_SECONDS}秒間に{RANSOMWARE_THRESHOLD}件以上のファイル変更を検知しました。\n" +
            ...
            try {
                ...
                if (s != null) {
                    _settings = new AppSettings { 
                        ...
                        LogRetentionDays = s.LogRetentionDays, MailConfig = s.MailConfig
                    };
                }
            } catch { }
        }
...
public class AppSettings { public List<BackupPair> BackupSettings { get; set; } = new(); public List<string> GlobalExclusions { get; set; } = new(); public int LogRetentionDays { get; set; } = 60; public MailSettings MailConfig { get; set; } = new(); }
public class AppSettingsRaw { public List<BackupPair> BackupSettings { get; set; } = new(); public List<string> GlobalExclusions { get; set; } = new(); public int LogRetentionDays { get; set; } = 60; public MailSettings MailConfig { get; set; } = new(); }
