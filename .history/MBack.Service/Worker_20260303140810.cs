...
using System.Security.AccessControl; // ★追加：権限操作用
...
public class Worker : BackgroundService
{
    ...
    private void SendDailySummary()
    {
        ...
        try {
            MailNotifier.SendMail(_settings.MailConfig, "【MBack】稼働サマリー", msg);
        } catch (Exception ex) {
    ...
    private bool CheckForRansomware()
    {
        ...
        if (_eventTimes.Count >= _settings.RansomwareThreshold)
        {
            ...
            NotifyError("SYSTEM_EMERGENCY", "ランサムウェア検知により緊急停止しました。");
            return true;
        }
        return false;
    }
    ...
    private void ProcessPendingBackups()
    {
        ...
        foreach (var kvp in _pendingBackups)
        {
            ...
            if ((now - pending.FirstDetected).TotalHours >= 24)
            {
                ...
                {
                    NotifyError(sourcePath, "24時間ロックのためスキップ");
                }
                continue;
            }
    ...
    private void PerformBackup(string sourcePath, BackupPair pair)
    {
        try {
            ...
            string owner = "System";
            try {
                var security = File.GetAccessControl(sourcePath);
                owner = security.GetOwner(typeof(NTAccount))?.ToString() ?? "System";
            } catch { }
            ...
            HistoryLogger.Log("Copy", sourcePath, new FileInfo(sourcePath).Length, "", owner);
        } catch (IOException) {
...
public static class MailNotifier
{
    public static void SendMail(MailSettings config, string subject, string body)
    {
        ...
        using var client = new SmtpClient(config.SmtpServer, config.SmtpPort) { 
            EnableSsl = config.SmtpSsl, Credentials = new NetworkCredential(config.UserName, config.Password) 
        };
    ...
    public static void SendErrorMail(MailSettings config, string filePath, string errorMessage) => SendMail(config, "【MBack】エラー", $"対象: {filePath}\n内容: {errorMessage}");
}
