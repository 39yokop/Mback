...
public class Worker : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        MountAllNetworkDrives(); // NASを接続
        StartWatchers(); // 監視を開始

        // 起動時に一度だけ古いテキストログを掃除（SQLite移行のため）
        MigrateOldLogs();
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                ProcessPendingBackups(); // バックアップ待ちのファイルを処理
            }
    ...
    private bool CheckForRansomware()
    {
        ...
        if (_eventTimes.Count >= _settings.RansomwareThreshold)
        {
            ...
            string msg = $"【緊急警告】短時間に大量の変更を検知したため、緊急停止しました。";
        ...
        try {
            foreach (var file in Directory.GetFiles(dir)) {
                ...
                if (!File.Exists(destPath) || File.GetLastWriteTimeUtc(file) > File.GetLastWriteTimeUtc(destPath)) {
                    QueueBackup(file, pair, true); // フルスキャンは顔パス(true)
                }
            }
    ...
    private void OnFileDeleted(string sourcePath, BackupPair pair)
    {
        ...
        try {
            ...
            NotifyError(sourcePath, "削除エラー: " + ex.Message);
        }
    }
